...
class DroneStation():
    ...
    def __init__(self, env, location):
        ...
        self.operation_centers = [simpy.Resource(env, capacity=14), simpy.Resource(env,capacity=5), simpy.Resource(env,capacity=2)]
...
class Order():
    ...
    def findClosestHub(self):
        ...
        """
        hub_locations = [(2000,2000),(2000,4000),(4000,4000),(4000,2000)]
...
class DroneCompany():
    ...
    def __init__(self, env):
        self.env = env
        self.hubs = [DroneStation(env, (2000,2000)), DroneStation(env, (2000,4000)), DroneStation(env, (4000,4000)), DroneStation(env, (4000,2000))]
    ...
    def getRelevantDroneType(self, order, policy):
        ...
        if policy == 1:
            ...
            if order.size == 1:
                if self.hubs[hub].operation_centers[0].count < 14:
                    return 1
                elif self.hubs[hub].operation_centers[1].count < 5:
                    return 2
                else:
                    return 1
            elif order.size == 2:
                if self.hubs[hub].operation_centers[1].count < 5:
                    return 2
                elif self.hubs[hub].operation_centers[2].count < 2:
                    return 3
                else:
                    return 2
            else:
        ...
        ########################### SECOND POLICY #########################
            ...
            centers = order.distancesFromHubs() # the hubs and their distances from the destination,
            # sorted from the closest to the farest
            if order.size == 1:
                for i in range(len(centers)): # try to enter to one of the basic drone operation centers
                    if self.hubs[centers[i][0]-1].operation_centers[0].count < 14:
                        order.hub_location = centers[i][0]
                        order.distance_from_hub = centers[i][1]
                        return 1
                for i in range(len(centers)): # try to enter to one of the regular drone operation center
                    if self.hubs[centers[i][0]-1].operation_centers[1].count < 5:
                        order.hub_location = centers[i][0]
                        order.distance_from_hub = centers[i][1]
                        return 2
                for i in range(len(centers)): # try to enter to one of the super drone operation center
                    if self.hubs[centers[i][0]-1].operation_centers[2].count < 2:
                        order.hub_location = centers[i][0]
                        order.distance_from_hub = centers[i][1]
                        return 3
                #Nothing is free? enter to the queue of the closest hub.
                order.hub_location = centers[0][0]
                order.distance_from_hub = centers[0][1]
                return 1
            elif order.size == 2:
                for i in range(len(centers)):
                    if self.hubs[centers[i][0]-1].operation_centers[1].count < 5: # try to enter to one of the regular drone operation center
                        order.hub_location = centers[i][0]
                        order.distance_from_hub = centers[i][1]
                        return 2
                for i in range(len(centers)):
                    if self.hubs[centers[i][0]-1].operation_centers[2].count < 2: # try to enter to one of the super drone operation center
                        order.hub_location = centers[i][0]
                        order.distance_from_hub = centers[i][1]
                        return 3
                # Nothing is free? enter to the queue of the closest hub.
...
def ordersArrival(env, drone_company):
    ...
    while True: # as long as new orders should arrive:
        current_time = env.now % (24 * 60) # catch current time
        if env.now == 0: # have we just started the simulation?
            yield env.timeout(5*60) # wait until 05:00AM
            continue
        elif drone_company.rate == 0 and env.now<(118*60): # are we in the time period 22:00-05:00?
