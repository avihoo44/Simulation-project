...
class DroneStation():
    ...
    def __init__(self, env, location):
        ...
        self.operation_centers = [simpy.Resource(env, capacity=68), simpy.Resource(env,capacity=26), simpy.Resource(env,capacity=11)]
...
class DroneCompany():
    ...
    def __init__(self, env):
        self.env = env
        self.hubs = [DroneStation(env, (3000,3000))]
    ...
    def getRelevantDroneType(self, order, policy):
        ...
        if policy == 1:
            ...
            if order.size == 1:
                if self.hubs[hub].operation_centers[0].count < 68:
                    return 1
                elif self.hubs[hub].operation_centers[1].count < 26:
                    return 2
                else:
                    return 1
            elif order.size == 2:
                if self.hubs[hub].operation_centers[1].count < 26:
                    return 2
                elif self.hubs[hub].operation_centers[2].count < 11:
                    return 3
                else:
                    return 2
            else:
