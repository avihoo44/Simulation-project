...
class DroneStation():
    ...
    def __init__(self, env, location):
        ...
        self.operation_centers = [simpy.Resource(env, capacity=7), simpy.Resource(env,capacity=3), simpy.Resource(env,capacity=1)]
...
class Order():
    ...
    def findClosestHub(self):
        ...
        """
        hub_locations = [(2000,2500),(2000,3500),(3000,4000),(4000,3500),(4000,2500),(3000,2000)]
...
class DroneCompany():
    ...
    def __init__(self, env):
        self.env = env
        self.hubs = [DroneStation(env, (2000,2500)), DroneStation(env, (2000,3500)), DroneStation(env, (3000,4000)), DroneStation(env, (4000,3500)), DroneStation(env, (4000,2500)), DroneStation(env, (3000,2000))]
    ...
    def getRelevantDroneType(self, order, policy):
        ...
        if policy == 1:
            ...
            if order.size == 1:
                if self.hubs[hub].operation_centers[0].count < 7:
                    return 1
                elif self.hubs[hub].operation_centers[1].count < 3:
                    return 2
                else:
                    return 1
            elif order.size == 2:
                if self.hubs[hub].operation_centers[1].count < 3:
                    return 2
                elif self.hubs[hub].operation_centers[2].count < 1:
                    return 3
                else:
                    return 2
            else:
